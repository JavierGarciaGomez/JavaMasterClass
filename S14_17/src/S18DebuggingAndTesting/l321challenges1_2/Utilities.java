...
public class Utilities {
    ...
    public char[] everyNthChar(char[] sourceArray, int n) {
        ...
        // if the array is null or the length is minor than n then return the same parameter
        if (sourceArray == null || sourceArray.length < n) {
            return sourceArray;
        }

        // divide for the new length, and create the temp array
        int returnedLength = sourceArray.length / n;
        char[] result = new char[returnedLength];
        int index = 0;

        // add a value each n positions
